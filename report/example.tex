...
\begin{itemize} 
	\item AnimatedBinaryTree(int[] arrInt), which is a constructor which takes an array of integers as a parameter and constructs an arraylist of Rect objects (rect_list), each value in arrInt would be used as a label for a Rect object in rect_list.
	...
	\item setColor(int index, Color c, String info), which is used to animate a change of colour for the Rect object at index position in rect_list. It will also use the String parameter info to pass to the stepForward method as the stepInfo parameter, as will all methods which take a String parameter info.
	\item swap(int a, int b, String info), which will swap the objects in index a, and index b, in both the rect_list and the graphical display.
	...
	\item add(int i), which adds the integer value i, as a Rect object, to the end of the AnimatedBinaryTree rect_list, and adds the Rect object to the graphical display.
	\item delete(int i), which removes the Rect object in index i from the rect_list and the graphical display, by first swapping the Rect object with the object in the last index of rect_list, and then removing it.
	\item modifyLabel(int n, String d, String info), which will change the label field of the Rect object at index n in rect_list, to the String parameter d.
