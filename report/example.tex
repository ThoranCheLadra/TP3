...
\begin{itemize}
	\item Design - Domain model diagrams to describe the components and classes of the project's API and explanation of the reason why the TimingFramework graphics framework was chosen over the other existing JAVA graphics frameworks.
	\item Implementation - this part will cover the tools and the methods used to code the API and issues encountered while doing it.
	...
	\item A user/reference manual documentation
\end{itemize}
...
In this chapter, we describe the implementation of the system.

%------------------------------------------------------------------------------
\section{User Interface}

Since we chose the API approach to build our Animated Algorithm, we do not really have a User Interface for building our animation. It is entirely up to the user of our deliverable to choose his own editor or programming environment.

We do have a Animation User Interface, which you can see at Appendix \ref{ui}.
...
\begin{itemize}
	...
	\end{itemize}
	\item Sections or contributions to sections Abstract, \ref{intro}, \ref{design}, \ref{PE:steps}, \ref{PE:refactoring}, \ref{conclusion}, \ref{Gediminas:Con} and \ref{maintenance} of this document
\end{itemize}
\subsection{Liam Bell}
\label{Liam:Con}
\begin{itemize}
	\item Graphics framework research
	\item Assisting other members with issues and proposing solutions
	\item Implementation of AnimatedBinaryTree class using existing step prototype
	\item Continuous development of AnimatedBinaryTree class to keep it in tune with the structure of the project
\end{itemize}
...
The delivered software implements and fulfills its main purpose and goal.

AnimatedArray data structure works fully as expected and described.

AnimatedLinkedList works as intended with a few deficiencies:
TODO list of deficiences

AnimatedBinaryTree works as intended with a few deficiencies:
\begin{itemize}
	\item The functionality to remove an element of the binary tree is unsupported. As a result the user may not remove elements from the binary tree.
	\item Adding elements to the binary tree which will cause it to require changing the y-spacing of elements in the graphical display, will cause errors to occur with the display. ie. the elements will displayed in the wrong positions, and lines linking nodes together will start appearing when they shouldn't.
\end{itemize}

Flash exporting currently works only for AnimatedArray.

When exporting using screenshots, screenshots taken from previous animation are not deleted.

Defining the window of the built animation does not work for AnimatedBinaryTree.
...
\begin{itemize}
\item add : takes 3 parameters: the index position to add the element to, the integer value of the element and a String to label the step
